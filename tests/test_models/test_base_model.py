...
class TestBaseModel(unittest.TestCase):
    ...
    def assert_hasattri(self, model, attribute) -> None:
        ...
        """
        self.assertTrue(
            hasattr(model, attribute), f"{model} Attribute {attribute} does not exist"
        )
    ...
    def assert_attribute_instance_types(self, model, attribute_dict: dict) -> None:
        ...
        for attribute_name, attribute_type in attribute_dict.items():
            self.assertIsInstance(
                ...
                attribute_type,
                f"Attribute {attribute_name} " f"not {attribute_type} instance",
            )
    ...
    def test_module_and_BaseModel_doc_string(self) -> None:
        ...
        self.assertIsNotNone(
            models.base_model.__doc__, f"Module {models.base_model} has no docstring"
        )
    ...
    def test_all_BaseModel_methods_doc_string(self) -> None:
        ...
        for name, method in BaseModel_method:
            self.assertIsNotNone(
                method.__doc__, f"Method <{name}> of BaseModel" f" has no docstring"
            )

    def test_pep8Compliance(self):
        style_guide = pycodestyle.StyleGuide()
        test_dir = os.path.dirname(os.path.abspath(__file__))
        file_path = os.path.join(test_dir, "../../models/base_model.py")
        report = style_guide.check_files([file_path])
        self.assertEqual(
            report.total_errors, 0, f"PEP8 violations found:" f" {report.total_errors}"
        )
    ...
    def test_init_method(self) -> None:
        ...
        self.assert_attribute_instance_types(
            self.model.__dict__,
    ...
    def test_custom_init_method(self) -> None:
        ...
        self.assertIsInstance(
            model_dict,
            BaseModel,
            "Deserialized object is " "not an instance of BaseModel",
        )
        self.assertIsInstance(
            model_json, dict, f"{model_json} " f"is not a dict instance"
        )

        # created_at and updated_at are datetime instance
        self.assert_attribute_instance_types(
            self.model.__dict__, {"created_at": datetime, "updated_at": datetime}
        )
    ...
    def test_to_dict_method(self) -> None:
        ...
        self.assert_attribute_instance_types(
            model_json, {"created_at": str, "updated_at": str}
        )
        self.assert_attribute_instance_types(
            self.model.__dict__, {"created_at": datetime, "updated_at": datetime}
        )
